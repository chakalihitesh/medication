    ...
    <header className="flex justify-between items-center w-full px-5 py-2 bg-surface shadow-sm sticky top-0 z-40 h-[68px] shrink-0 border-b border-surface-variant/30">
      <div className="flex items-center gap-3 w-1/4">
        {showBack && onBack ? (
          ...
          >
            {(title === "FAMILY HEALTH" || title === "personal Info" || title === "Edit Profile") ? <ChevronLeft size={28} /> : <ArrowLeft size={24} />}
          </button>
        ) : (
          <img 
      ...
      <div className="flex-1 flex flex-col justify-center items-center">
        ...
        {title === "personal Info" ? (
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-[22px] font-bold text-[#0052cc] italic tracking-tight text-center">{title}</h1>
          </div>
        ) : (
          <h1 className={cn(
            "tracking-tight text-center",
            title === "FAMILY HEALTH" ? "text-base font-semibold text-[#0052cc]" : "text-[22px] font-bold text-primary"
          )}>
            {title || "HealthMate AI"}
          </h1>
        )}
