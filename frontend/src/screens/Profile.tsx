...
import { User, Users, Edit2, BookOpen, HelpCircle, LogOut, ChevronRight, BadgeCheck, Camera, Eye, X, Globe, MapPin } from 'lucide-react';
...
export function Profile({ setScreen, onLanguageChange }: ProfileProps) {
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-4xl mx-auto pb-24 h-full overflow-y-auto px-5 pt-6 relative">
        ...
        <div className="bg-surface-container-lowest rounded-xl shadow-[0_4px_20px_rgba(0,61,155,0.04)] overflow-hidden flex flex-col divide-y divide-surface-variant/30">
          ...
          >
            ...
            <div className="ml-4 flex-1">
              <p className="text-base text-on-surface font-medium group-hover:text-primary transition-colors">{t('personal_info', 'Personal Info')}</p>
            </div>
          ...
          >
            ...
            <div className="ml-4 flex-1">
              <p className="text-base text-on-surface font-medium group-hover:text-primary transition-colors">{t('family_health', 'Family Health')}</p>
            </div>
            <ChevronRight size={20} className="text-outline-variant group-hover:translate-x-1 transition-transform" />
          </button>
          ...
          >
            ...
            <div className="ml-4 flex-1">
              <p className="text-base text-on-surface font-medium group-hover:text-secondary transition-colors">{t('edit_profile', 'Edit Profile')}</p>
            </div>
            <ChevronRight size={20} className="text-outline-variant group-hover:translate-x-1 transition-transform" />
          </button>
          ...
          <button 
            type="button"
            onClick={() => setScreen?.('location-services')}
            className="flex items-center p-4 w-full text-left hover:bg-surface-container-low transition-all active:scale-[0.98] group"
          >
            <div className="w-10 h-10 rounded-xl bg-[#e8f0fe] flex items-center justify-center text-[#1a73e8] shrink-0 group-hover:scale-110 transition-transform">
              <MapPin size={22} className="fill-current animate-bounce" />
            </div>
            <div className="ml-4 flex-1">
              <p className="text-base text-on-surface font-medium group-hover:text-[#1a73e8] transition-colors">{t('nearby_specialists', 'Nearby Specialists')}</p>
              <p className="text-[11px] text-on-surface-variant leading-none mt-0.5">{t('find_specialists_desc', 'Interactive map & specialist search')}</p>
            </div>
            <ChevronRight size={20} className="text-outline-variant group-hover:translate-x-1 transition-transform" />
          </button>
          ...
          <button className="flex items-center p-4 w-full text-left hover:bg-surface-container-low transition-all active:scale-[0.98] group">
            ...
            <div className="ml-4 flex-1">
              <p className="text-base text-on-surface font-medium group-hover:text-tertiary transition-colors">{t('guidelines', 'Guidelines')}</p>
            </div>
            <ChevronRight size={20} className="text-outline-variant group-hover:translate-x-1 transition-transform" />
          </button>
          ...
          <button className="flex items-center p-4 w-full text-left hover:bg-surface-container-low transition-all active:scale-[0.98] group">
            ...
            <div className="ml-4 flex-1">
              <p className="text-base text-on-surface font-medium">{t('help_support', 'Help & Support')}</p>
            </div>
            <ChevronRight size={20} className="text-outline-variant group-hover:translate-x-1 transition-transform" />
          </button>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 gap-3">
            <div className="flex items-center">
              <div className="w-10 h-10 rounded-xl bg-primary-fixed/20 flex items-center justify-center text-primary shrink-0">
                <Globe size={22} className="fill-current" />
              </div>
              <div className="ml-4">
                <p className="text-base text-on-surface font-medium">{t('language_preference', 'Language Preference')}</p>
                <p className="text-xs text-on-surface-variant">{t('choose_language', 'Choose your AI voice & chat language')}</p>
              </div>
            </div>
            <div className="flex items-center gap-1.5 bg-surface-container rounded-lg p-0.5 border border-surface-variant/30 w-fit self-end sm:self-auto">
              {[
      ...
      {/* Deleted inline location search - replaced by dedicated sub-screen button under settings list */}
      ...
      <button 
        ...
        className="mt-4 mb-8 w-full bg-surface-container-lowest text-error font-semibold text-xl py-4 rounded-xl shadow-[0_4px_12px_rgba(186,26,26,0.08)] border border-error/10 flex items-center justify-center gap-3 hover:bg-error-container/10 active:scale-[0.97] transition-all"
      >
         <LogOut size={24} />
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-[fade-in_0.2s_ease-out]">
          <div className="bg-white w-full max-w-sm rounded-[24px] p-5 shadow-2xl animate-[slide-up_0.3s_ease-out]" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-[20px] font-bold text-[#1a1a1a]">{t('photo_title', 'Profile Photo')}</h3>
            ...
            <div className="flex flex-col gap-3">
              ...
              >
                ...
                <span className="text-[17px] font-semibold text-[#1a1a1a] group-hover:text-[#0052cc] transition-colors">{t('see_photo', 'See Photo')}</span>
              </button>
              ...
              >
                ...
                <span className="text-[17px] font-semibold text-[#1a1a1a] group-hover:text-[#00687a] transition-colors">{t('change_photo', 'Change Photo')}</span>
              </button>
