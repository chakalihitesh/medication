...
with tab_meds:
    ...
    with st.form("new_med_form", clear_on_submit=True):
        ...
        with col_n2:
            ...
            med_freq = st.selectbox("Frequency", ["Daily", "Twice a Day", "Three times a Day", "Weekly", "As Needed"])
            ...
            if med_name and med_dosage:
                def add_entry(t_str):
                    ...
                    if hour >= 12 and hour < 17:
                        ...
                        sched_cat = "morning"
                    database.add_medication(med_name, t_str, med_dosage, med_freq, sched_cat)

                if med_freq == "Twice a Day":
                    add_entry("08:00")
                    add_entry("20:00")
                    st.success(f"Successfully scheduled {med_name} (Twice a day at 08:00 & 20:00)!")
                elif med_freq == "Three times a Day":
                    add_entry("08:00")
                    add_entry("12:00")
                    add_entry("20:00")
                    st.success(f"Successfully scheduled {med_name} (Three times a day at 08:00, 12:00 & 20:00)!")
                else:
                    time_str = med_time.strftime("%H:%M")
                    add_entry(time_str)
                    st.success(f"Successfully scheduled {med_name} at {time_str}!")
                st.rerun()
            else:
